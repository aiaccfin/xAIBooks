...
class PGHandler:
    ...
    def get_biz(self, table_name='xai', biz_id=None):
        try:
            query = (f"SELECT * FROM {table_name} "
                     f"WHERE (biz_id = %s OR biz_id IS NULL) ")
    ...
    def copy_default_coa_to_biz(self, biz_type, biz_id):
        ...
        try:
            ...
            '''
            print(f"Type of biz_type: {type(biz_type)}")
    ...
    def get_biz_coa(self, table_name='biz_coa', biz_id=801):
        try:
            query = (f"""
                SELECT * 
                FROM biz_coa
